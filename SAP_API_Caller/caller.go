...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-sales-quotation-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(salesQuotation string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerData, "function": "SalesQuotationHeader"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerPartnerData, "function": "SalesQuotationHeaderPartner"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "SalesQuotationItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemPricingElementData, "function": "SalesQuotationItemPricingElement"})
	if err != nil {
		c.log.Error(err)
		return
	}
